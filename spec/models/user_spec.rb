...
RSpec.describe User, type: :model do
  describe '正常にユーザー登録できるか' do
    it 'Twitterからユーザー登録' do
      user = FactoryBot.build(:user, :twitter)
    ...
    it 'ゲストユーザーで登録' do
      user = FactoryBot.build(:user, :guest)
      expect(user.valid?).to be true
  ...
  describe 'バリデーション確認' do
    context 'presence: trueのバリデーション' do
      it 'twitter_idが未入力のuserは登録できない' do
        user = FactoryBot.build(:user, :twitter, twitter_id: nil)
        expect(user.valid?).to be false
        expect(user.errors.full_messages).to include("Twitter IDを入力してください")
      end
  
      it '名前が未入力のuserは登録できない' do
        user = FactoryBot.build(:user, :twitter, screen_name: nil)
        expect(user.valid?).to be false
    ...
    context 'uniqueness: trueのバリデーション' do
      it '名前が未入力のuserは登録できない' do
        user = FactoryBot.create(:user, :twitter, email: 'test@example')
        user_with_duplicate_email = FactoryBot.build(:user, :twitter, email: 'test@example')
